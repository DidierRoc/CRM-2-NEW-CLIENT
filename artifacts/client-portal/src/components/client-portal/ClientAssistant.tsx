...
const ClientAssistant = ({
  ...
  const dynamicSuggestions = useMemo(() => {
    ...
    if (portfolio?.totalValue && portfolio.totalValue > 0) {
      list.push(`Combien aurai-je dans 12 mois ?`);
    }
    if (portfolio?.activeSubs?.length) {
      list.push(`Explique-moi mes ${portfolio.activeSubs.length} contrat${portfolio.activeSubs.length > 1 ? 's' : ''}`);
    }
    if (portfolio?.totalInterests && portfolio.totalInterests > 0) {
      list.push(`Quelle est ma performance actuelle ?`);
    }
    list.push(`Que se passe-t-il si j'investis 500 € de plus par mois ?`);
    if (portfolio?.totalValue) {
      const half = Math.round(portfolio.totalValue / 2 / 100) * 100;
      if (half > 0) list.push(`Puis-je retirer ${fmtEUR(half)} ?`);
    }
  ...
  const sendToAdvisor = async (content: string) => {
    try {
      ...
      setMessages((prev) => [...prev, { role: 'assistant', content: 'Votre message a bien été envoyé à votre conseiller.' }]);
      ...
      setMessages((prev) => [...prev, { role: 'assistant', content: 'La transmission au conseiller est momentanément indisponible. Vous pouvez réessayer depuis Ma messagerie.' }]);
    }
  };
  ...
  const send = async (text: string, source: 'typed' | 'suggestion' = 'typed') => {
    ...
    try {
      ...
      const rawReply = data?.reply || data?.error || "Désolé, je n'ai pas pu répondre.";
      ...
      setMessages((prev) => [...prev, { role: 'assistant', content: reply || 'Souhaitez-vous envoyer ce message à votre conseiller ?' }]);
      ...
      setMessages((prev) => [
        ...
        {
          role: 'assistant',
          content: "Le service est momentanément indisponible. Réessayez dans un instant.",
        },
      ]);
    } finally {
      setLoading(false);
        ...
        >
          ...
          >
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="font-semibold text-white text-sm">Assistant patrimonial</h3>
                <p className="text-[11px] text-white/70 flex items-center gap-1.5">
                  <span className="w-1.5 h-1.5 rounded-full bg-emerald-400 animate-pulse" />
                  En ligne — conseil uniquement
                </p>
              </div>
            </div>
            <button
          ...
          <div ref={scrollRef} className="flex-1 overflow-y-auto p-4 space-y-3 bg-slate-50/50">
            ...
            {messages.length <= 1 && !loading && (
              <div className="pt-2 space-y-1.5">
                <p className="text-[11px] font-medium text-slate-500 px-1">Suggestions personnalisées</p>
            ...
            {pendingAdvisorMessage && !loading && (
              <div className="ml-9 rounded-xl border border-slate-200 bg-white p-3 shadow-sm space-y-2">
                <p className="text-xs text-slate-600">Souhaitez-vous envoyer ce message à votre conseiller ?</p>
                <div className="flex gap-2">
          ...
          <p className="px-3 pb-2 text-[10px] text-center text-slate-400 bg-white">
            ⓘ Les réponses sont indicatives et ne constituent pas un conseil en investissement.
          </p>
        </div>
      )}
    </>
