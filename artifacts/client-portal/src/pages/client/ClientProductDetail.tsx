...
const ClientProductDetail = () => {
  ...
  const handleOpenSignedContract = async () => {
    if (!activeContract) {
      toast.error('Aucun contrat signé trouvé');
      return;
    }
  ...
  const handleDownloadSignedContract = async () => {
    ...
    const t = toast.loading('Préparation du contrat signé…');
    try {
      ...
      if (pdfUrl && activeContract.signature_data) {
        ...
        toast.success('Contrat signé téléchargé', { id: t });
        return;
      }
  ...
  const handleSubscribe = async () => {
    ...
    if (normalizedAmount < amountLimits.min || normalizedAmount > amountLimits.max) {
      toast.error(`Le montant doit être entre ${amountLimits.min}€ et ${amountLimits.max}€`);
      return;
    }
    ...
    try {
      ...
      setStep('done');
      toast.success('Félicitations, votre contrat a été signé avec succès.');
    } catch (err: any) {
      ...
      toast.error('Erreur lors de la signature du contrat: ' + (err.message || 'Erreur inconnue'));
    } finally {
      setSigning(false);
    }
  };
  ...
  return (
    <div className="max-w-5xl space-y-5 animate-in fade-in-0 duration-200">
      <Button variant="ghost" size="sm" onClick={() => navigate('/client/products')} className="text-muted-foreground hover:text-foreground -ml-1">
        <ArrowLeft className="w-4 h-4 mr-1.5" />Retour aux produits
      </Button>
          ...
          <div className="flex items-start justify-between">
            ...
            <div className="flex items-center gap-2 flex-wrap justify-end">
              <span className="inline-flex items-center gap-1.5 text-[10px] font-semibold px-2.5 py-1.5 rounded-full bg-[#c9a84c]/25 text-[#f5d98a] ring-1 ring-[#c9a84c]/30 backdrop-blur-sm">
                <ShieldCheck className="w-3 h-3" />Dépôts protégés
              </span>
          ...
          <div className="space-y-1">
              ...
              <div className="pt-1">
                <div className="flex items-end gap-3">
                  ...
                  <div className="flex flex-col pb-1">
                    ...
                    <span className="text-white/55 text-xs font-bold uppercase tracking-[0.2em] leading-tight">/ AN</span>
                  </div>
                </div>
                <div className="flex items-center gap-2 mt-2.5 flex-wrap">
                  <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full bg-white/15 ring-1 ring-white/20 text-white/90 text-[11px] font-semibold backdrop-blur-sm">
                    <ShieldCheck className="w-3 h-3" /> Taux fixe garanti
                  </span>
                  {product.duree && (
                    <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full bg-white/15 ring-1 ring-white/20 text-white/90 text-[11px] font-semibold backdrop-blur-sm">
                  ...
                  {isLocked && (
                    <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full bg-orange-500/20 ring-1 ring-orange-400/30 text-orange-200 text-[11px] font-semibold backdrop-blur-sm">
                      <Lock className="w-3 h-3" /> Capital bloqué jusqu'à échéance
                    </span>
                  )}
                </div>
              </div>
            ...
            {leadProduct?.created_at && (
              <p className="text-white/40 text-[10px] pt-1">
                Souscrit le {new Date(leadProduct.created_at).toLocaleDateString('fr-FR', { day: 'numeric', month: 'long', year: 'numeric' })}
              </p>
            )}
          </div>
        </div>
      ...
      <div className="grid grid-cols-3 gap-3">
        <div className="bg-gradient-to-br from-emerald-50 to-teal-50 dark:from-emerald-950/40 dark:to-teal-950/40 border border-emerald-100 dark:border-emerald-800/40 rounded-2xl p-4 flex flex-col items-center gap-1.5 shadow-sm">
          ...
          <p className="text-[10px] text-emerald-700/60 dark:text-emerald-300/60 font-semibold uppercase tracking-wide">Taux annuel</p>
        ...
        <div className="bg-gradient-to-br from-blue-50 to-indigo-50 dark:from-blue-950/40 dark:to-indigo-950/40 border border-blue-100 dark:border-blue-800/40 rounded-2xl p-4 flex flex-col items-center gap-1.5 shadow-sm">
          ...
          <p className="text-[10px] text-blue-700/60 dark:text-blue-300/60 font-semibold uppercase tracking-wide">Durée</p>
        ...
        {isLocked ? (
          <div className="bg-gradient-to-br from-orange-50 to-red-50 dark:from-orange-950/40 dark:to-red-950/40 border border-orange-100 dark:border-orange-800/40 rounded-2xl p-4 flex flex-col items-center gap-1.5 shadow-sm">
            ...
            <p className="text-[10px] text-orange-700/60 dark:text-orange-300/60 font-semibold uppercase tracking-wide">Fonds</p>
            <p className="text-sm font-black text-orange-700 dark:text-orange-300 text-center leading-tight">Bloqués</p>
          </div>
        ) : (
          <div className="bg-gradient-to-br from-violet-50 to-purple-50 dark:from-violet-950/40 dark:to-purple-950/40 border border-violet-100 dark:border-violet-800/40 rounded-2xl p-4 flex flex-col items-center gap-1.5 shadow-sm">
            ...
            <p className="text-[10px] text-violet-700/60 dark:text-violet-300/60 font-semibold uppercase tracking-wide">Fonds</p>
            <p className="text-sm font-black text-violet-700 dark:text-violet-300 text-center leading-tight">Disponibles</p>
          </div>
        )}
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
          ...
          },
          {
            ...
            label: isLocked ? 'Capital bloqué' : 'Capital disponible',
            value: product.duree || '—',
            bg: isLocked
      ...
      {(product.prix_minimum || product.prix_maximum) && (
        <div className="bg-gradient-to-br from-amber-50 to-orange-50 dark:from-amber-950/40 dark:to-orange-950/40 border border-amber-100 dark:border-amber-800/40 rounded-2xl p-4 flex items-center gap-4 shadow-sm">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-[10px] text-amber-700/60 dark:text-amber-300/60 font-semibold uppercase tracking-wide mb-2">Investissement</p>
            <div className="flex items-center gap-4 flex-wrap">
              <div className="flex flex-col">
                ...
                <span className="text-xs text-amber-600/70 dark:text-amber-400/70 mt-0.5">minimum</span>
              </div>
              {product.prix_maximum && (
                <>
                  ...
                  <div className="flex flex-col">
                    ...
                    <span className="text-xs text-amber-600/70 dark:text-amber-400/70 mt-0.5">maximum</span>
                  </div>
                </>
              )}
            </div>
      ...
      <div className="rounded-2xl border border-border/60 bg-card overflow-hidden shadow-sm">
        <div className="flex items-center gap-3 p-4 border-b border-border/60">
          ...
          <span className="font-semibold text-foreground text-sm">Indicateurs financiers</span>
        </div>
        <div className="divide-y divide-border/60">
          {[
            { label: 'Taux annuel net', value: product.interets || '—' },
            { label: 'Durée', value: product.duree || '—' },
            ...(product.prix_minimum ? [{ label: 'Versement minimum', value: `${product.prix_minimum.toLocaleString('fr-FR')} €` }] : []),
            ...(product.prix_maximum ? [{ label: 'Versement maximum', value: `${product.prix_maximum.toLocaleString('fr-FR')} €` }] : []),
            { label: 'Disponibilité des fonds', value: isLocked ? 'Bloqués jusqu\'à échéance' : 'Disponibles à tout moment' },
            { label: 'Paiement des intérêts', value: isLocked ? 'À l\'échéance contractuelle' : 'Périodique selon contrat' },
            { label: 'Garantie des dépôts', value: 'Jusqu\'à 100 000 € (SGD Luxembourg)' },
          ].map(({ label, value }) => (
      ...
      <div className="rounded-2xl border border-border/60 bg-card overflow-hidden shadow-sm">
        <div className="flex items-center gap-3 p-4 border-b border-border/60">
          ...
          <span className="font-semibold text-foreground text-sm">Chronologie du placement</span>
        </div>
        <div className="p-5">
          <div className="flex items-start gap-0">
            {[
              { label: 'Souscription', sub: 'Signature du contrat', color: 'bg-primary', textColor: 'text-primary' },
              { label: 'Placement actif', sub: isLocked ? 'Fonds bloqués' : 'Fonds disponibles', color: 'bg-blue-500', textColor: 'text-blue-600 dark:text-blue-400' },
              { label: product.duree ? (isLocked ? `${product.duree} de blocage` : `Durée : ${product.duree}`) : 'Durée contractuelle', sub: isLocked ? 'Capital immobilisé' : 'Capital valorisé', color: 'bg-amber-500', textColor: 'text-amber-600 dark:text-amber-400' },
              { label: 'Versement final', sub: 'Capital + intérêts', color: 'bg-emerald-500', textColor: 'text-emerald-600 dark:text-emerald-400' },
            ].map((step, i, arr) => (
      ...
      {_rate > 0 && (
        <div className="rounded-xl bg-gradient-to-br from-emerald-50 to-teal-50 dark:from-emerald-950/40 dark:to-teal-950/40 border border-emerald-200 dark:border-emerald-800/50 p-4 flex items-center justify-between gap-4">
          <div className="min-w-0">
            <p className="text-xs text-emerald-700/70 dark:text-emerald-400/70 font-semibold uppercase tracking-wide mb-1">Exemple de simulation</p>
            <p className="text-sm text-emerald-800 dark:text-emerald-300 leading-snug">
              Pour un dépôt de{' '}
              <strong>{_simAmount.toLocaleString('fr-FR')} €</strong>{' '}
              sur {_months} mois, vous percevrez environ{' '}
              <strong>{_interests.toLocaleString('fr-FR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} €</strong> d'intérêts nets.
            </p>
          </div>
          <div className="flex-shrink-0 text-right">
            ...
            <p className="text-[10px] text-emerald-600/70 dark:text-emerald-400/70 font-semibold mt-0.5">capital final récupéré</p>
          </div>
        </div>
      )}
      ...
      <div className="bg-card border border-border/60 rounded-2xl overflow-hidden shadow-sm">
        <div className="flex items-center gap-3 p-4 border-b border-border/60">
          ...
          <span className="font-semibold text-foreground text-sm">Simulateur de rendement</span>
        </div>
        <div className="p-4">
      ...
      <div className="lg:sticky lg:top-6 space-y-4">
        ...
        <div className="rounded-2xl overflow-hidden shadow-md border border-border/50">
          <div className="px-5 py-4 flex items-center gap-2.5" style={{ background: 'linear-gradient(135deg, #0f2347 0%, #1a3a6b 100%)' }}>
            ...
            <span className="text-white font-semibold text-sm tracking-wide">Pourquoi choisir ce placement ?</span>
          </div>
        ...
        <div className="rounded-xl border border-[#c9a84c]/30 bg-amber-50/50 dark:bg-amber-950/20 p-4 flex items-start gap-3">
          ...
          <div>
            <p className="text-xs font-bold text-amber-800 dark:text-amber-300">Dépôts protégés</p>
            <p className="text-xs text-amber-700/80 dark:text-amber-400/80 mt-0.5 leading-relaxed">
              Conformément au système luxembourgeois de garantie des dépôts (SGD), vos avoirs sont couverts jusqu'à <strong>100 000 €</strong> par établissement.
            </p>
          </div>
        </div>
        ...
        {product.description && (
          <details className="rounded-2xl border border-border/50 bg-card overflow-hidden shadow-sm group">
            <summary className="px-5 py-3.5 flex items-center gap-2.5 cursor-pointer list-none select-none hover:bg-muted/30 transition-colors">
              ...
              <span className="text-sm font-semibold text-foreground flex-1">Voir les conditions complètes</span>
      ...
      {alreadySubscribed ? (
        <div className="bg-card border border-border/60 rounded-2xl p-5 shadow-sm">
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1 min-w-0">
              <p className="font-bold text-foreground">Souscription active</p>
              <p className="text-sm text-muted-foreground mt-0.5">
                {existingSub.amount ? `${existingSub.amount.toLocaleString('fr-FR')} € placés` : ''}{existingSub.amount ? ' · ' : ''}
                {existingSub.status === 'pending_payment' ? 'En attente de versement' :
                 existingSub.status === 'active' ? 'Actif' : 'Signé'}
              </p>
            </div>
          </div>
          <div className="mt-4 pt-4 border-t border-border/60 flex flex-wrap gap-2">
            ...
            <Button variant="outline" size="sm" onClick={() => navigate('/client/contracts')} className="rounded-xl">
              <FileText className="w-4 h-4 mr-1.5" />Mes contrats
            </Button>
          </div>
        </div>
      ) : (
        <div className="bg-card border border-border/60 rounded-2xl overflow-hidden shadow-sm">
          ...
          {step === 'info' && (
            <div className="p-6 sm:p-8 space-y-6">
              ...
              <div className="text-center space-y-2">
                <h2 className="font-black text-foreground text-xl">Prêt à investir ?</h2>
                {_rate > 0 && _simAmount > 0 && (
                  <p className="text-sm text-muted-foreground">
                    Pour <strong className="text-foreground">{_simAmount.toLocaleString('fr-FR')} €</strong> investis,
                    vous récupérerez{' '}
                    <strong className="text-emerald-600 dark:text-emerald-400">
                      {_final.toLocaleString('fr-FR', { minimumFractionDigits: 0, maximumFractionDigits: 0 })} €
                    </strong>{' '}
                    au terme des {_months} mois.
                  </p>
                )}
              </div>
              ...
              <div className="flex items-center justify-center gap-4 text-sm">
                ...
                >
                  <TrendingUp className="w-3.5 h-3.5" />Simuler mon rendement
                </button>
              </div>
              ...
              <div className="flex items-center justify-center gap-4 pt-2 border-t border-border/60">
                <span className="inline-flex items-center gap-1 text-[11px] text-muted-foreground">
                  <ShieldCheck className="w-3 h-3 text-emerald-500" /> Signature électronique sécurisée
                </span>
                <span className="inline-flex items-center gap-1 text-[11px] text-muted-foreground">
                  <Lock className="w-3 h-3 text-primary" /> Contrat horodaté
                </span>
              </div>
            </div>
          )}
          ...
          {step === 'amount' && (
            <div className="p-6 space-y-5">
              <h2 className="text-lg font-semibold text-foreground">Définir le montant d'investissement</h2>
              <div className="space-y-3">
                ...
                {(normalizedAmount < amountLimits.min || normalizedAmount > amountLimits.max) && (
                  <p className="text-xs text-destructive">Le montant doit être entre {amountLimits.min}€ et {amountLimits.max}€</p>
                )}
              </div>
              ...
              <div className="border rounded-lg p-4 space-y-4">
                <div className="flex items-center gap-3">
                  ...
                  <label htmlFor="joint" className="text-sm font-medium text-foreground cursor-pointer flex items-center gap-2">
                    ...
                    Compte commun (2 souscripteurs)
                  </label>
                </div>

                {isJointAccount && (
                  <div className="space-y-3 pt-2 border-t">
                    <p className="text-xs text-muted-foreground font-medium uppercase tracking-wider">Identité du co-souscripteur</p>
                    <div className="grid grid-cols-2 gap-3">
                      ...
                      <div>
                        ...
                        <Input value={coSubscriber.nationalite} onChange={e => setCoSubscriber(p => ({ ...p, nationalite: e.target.value }))} placeholder="Française" />
                      </div>
                      <div>
              ...
              <div className="flex gap-2">
                ...
                >
                  {loadingContract ? <><Loader2 className="w-4 h-4 mr-2 animate-spin" />Chargement...</> : 'Voir le contrat'}
                </Button>
              </div>
            </div>
          )}
          ...
          {step === 'contract' && (
            <div className="p-6 space-y-5">
              <h2 className="text-lg font-semibold text-foreground">Aperçu du contrat</h2>
              <div className="bg-muted/50 rounded-lg p-0 max-h-[600px] overflow-y-auto">
                <ContractHtmlFrame title="Aperçu du contrat" html={contractHtml} className="w-full bg-white border-0" />
              </div>
              <div className="flex items-start gap-2">
                ...
                <label htmlFor="accept" className="text-sm text-muted-foreground leading-snug cursor-pointer">
                  J'ai lu et j'accepte les conditions générales du contrat de souscription et je confirme vouloir investir <strong>{normalizedAmount.toLocaleString('fr-FR')}€</strong> dans <strong>{product.nom}</strong>.
                </label>
              </div>
              <div className="flex gap-2">
                <Button variant="outline" onClick={() => { setStep('amount'); setAccepted(false); }}>Retour</Button>
                <Button onClick={() => setStep('sign')} disabled={!accepted || !canSignCurrentPreview}>Signer le contrat</Button>
              </div>
            </div>
          )}
          ...
          {step === 'sign' && (
            <div className="p-6 space-y-5">
              <p className="text-sm text-foreground leading-relaxed">
                J'ai lu et j'accepte les conditions générales du contrat de souscription et je confirme vouloir investir la somme de <strong>{normalizedAmount.toLocaleString('fr-FR')} €</strong> dans <strong>{product.nom}</strong>.
              </p>
              <SignatureCanvas onSignatureChange={setSignatureData} />
              <div className="flex gap-2">
                <Button variant="outline" onClick={() => setStep('contract')}>Retour</Button>
                <Button onClick={handleSubscribe} disabled={!signatureData || signing}>
                  {signing ? <><Loader2 className="w-4 h-4 mr-2 animate-spin" />Validation...</> : 'Valider et signer le contrat'}
                </Button>
              </div>
            </div>
          )}
          ...
          {step === 'done' && (
            <div className="p-6 sm:p-8 space-y-6">
              ...
              <div className="text-center space-y-3">
                ...
                <h2 className="text-2xl font-bold text-foreground">Contrat signé avec succès</h2>
                <p className="text-sm text-muted-foreground max-w-md mx-auto leading-relaxed">
                  {leadData?.civilite} {leadData?.nom}, félicitations pour la signature de votre contrat. Votre exemplaire signé est prêt à être téléchargé.
                </p>
              </div>
              ...
              {activeContract && (
                <div className="border border-border rounded-2xl overflow-hidden max-w-lg mx-auto shadow-sm">
                  ...
                  <div className="bg-primary/5 border-b border-border px-5 py-4 flex items-center gap-3">
                    ...
                    <div className="min-w-0">
                      <p className="font-semibold text-foreground text-sm truncate">Contrat — {product?.nom}</p>
                      <p className="text-xs text-muted-foreground">
                        Signé le {new Date(activeContract.signed_at || activeContract.created_at || Date.now()).toLocaleDateString('fr-FR', { day: 'numeric', month: 'long', year: 'numeric' })}
                      </p>
                    </div>
                  ...
                  <div className="px-5 py-4 space-y-2 bg-card">
                    <div className="flex justify-between text-sm">
                      <span className="text-muted-foreground">Investisseur</span>
                      <span className="font-medium text-foreground">{leadData?.prenom} {leadData?.nom}</span>
                    </div>
                    <div className="flex justify-between text-sm">
                      <span className="text-muted-foreground">Montant souscrit</span>
                  ...
                  <div className="px-5 py-4 border-t border-border bg-card space-y-2">
                    ...
                    >
                      {downloadingPdf ? (
                        <><Loader2 className="w-4 h-4 mr-2 animate-spin" />Préparation du document…</>
                      ) : (
                        <><Download className="w-4 h-4 mr-2" />Télécharger mon contrat signé (PDF)</>
                      )}
                    </Button>
                    <Button variant="ghost" size="sm" className="w-full text-muted-foreground" onClick={handleOpenSignedContract}>
                      <Eye className="w-3.5 h-3.5 mr-1.5" />Aperçu du contrat
                    </Button>
                  </div>
                </div>
              )}
              ...
              <div className="flex flex-wrap justify-center gap-3 pt-2">
                <Button variant="outline" onClick={() => navigate('/client/products')}>Retour aux produits</Button>
                <Button onClick={() => navigate('/client/contracts')}>
                  <FileText className="w-4 h-4 mr-2" />Voir mes contrats
                </Button>
              </div>
            </div>
          )}
      ...
      <Dialog open={contractDialogOpen} onOpenChange={setContractDialogOpen}>
        ...
        >
          <DialogHeader>
            <DialogTitle>Contrat actif</DialogTitle>
          </DialogHeader>

          {loadingContractView ? (
            <div className="flex items-center justify-center py-8 text-muted-foreground">
              <Loader2 className="w-5 h-5 animate-spin mr-2" />Chargement du contrat...
            </div>
          ) : (
            <div className="space-y-4">
              {viewContractHtml ? (
                ...
                <p className="text-sm text-muted-foreground">Le contrat signé est indisponible pour le moment.</p>
              )}
              ...
              {activeContract && (
                <div className="flex justify-end pt-2 border-t">
                  <Button variant="outline" size="sm" onClick={handleDownloadSignedContract} disabled={downloadingPdf}>
                    {downloadingPdf ? (
                      <><Loader2 className="w-3.5 h-3.5 mr-1 animate-spin" />Préparation…</>
                    ) : activeContract.contract_pdf_url ? (
                      <><Download className="w-3.5 h-3.5 mr-1" />Télécharger PDF</>
                    ) : (
                      <><Download className="w-3.5 h-3.5 mr-1" />Générer le PDF</>
                    )}
                  </Button>
                </div>
              )}
